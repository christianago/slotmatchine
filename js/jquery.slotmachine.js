...
$(window).load(function(){
	...
	for(var i = 1; i <= totalMachines; i++){
		machines[i-1] = $("#machine"+i).slotMachine({
			active	: i,
			delay	: 500,
			randomize : function(){ return myrandomize(); }
		});
	}
	

	$(document).on('click', '#slotMachineButton1', function(e){
		
		var myRandom = Math.random();
		TIMES++;
		
		if ( myRandom < PROBABILITY ){
			WIN = true;
			++TOTALWINS;
			CURRENT_WIN = Math.floor(myRandom * totalFruits);
		} else WIN = false;
	...
	function postpone(machine, delay){
		setTimeout(function(){ machine.shuffle(5, onComplete); }, delay);
	}
	...
	function myrandomize(){
		if ( WIN ){
			...
			if ( slots.length == totalMachines+1 ){
				slots = [];
			}
			var myrnd = Math.floor(Math.random() * totalFruits);
			slots.push(myrnd);
			if ( slots.length == 2 && slots[0] == slots[1] ){
				myrnd = slots[1] + 1;
				if ( slots[1] == totalFruits ){
					myrnd = 0;
				}
				//console.log(slots, myrnd);
				return myrnd;
			}
	...
	function onComplete($el, active){
		...
		for(var i = 1; i <= totalMachines; i++){
			if ( $el[0].id == 'machine'+i ){
				$("#machine"+i+"Result").text("Index: "+active.index);
				continue;
			}
		}
		
		if ( $el[0].id == 'machine'+totalMachines ){
			//setTimeout(function(){ $("#slotMachineButton1").trigger('click');  }, 200);
		}
